...
Создадим еще один дочерний класс ***Greenhouse***:
```
...
```
Дочерний класс расширяется с помощью поля ```window```, который в свою очередь является объектом другого клаcса ***Window***:
